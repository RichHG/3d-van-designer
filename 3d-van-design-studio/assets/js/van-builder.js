...
init() {
    if (!vanBuilderData || !vanBuilderData.availableModels) {
        console.error('vanBuilderData is not properly initialized');
        return;
    }
    ...
    // Add this code to ensure grid is visible
    ...
    // Create a dummy UIControls if the real one isn't available
    try {
        if (typeof UIControls !== 'undefined') {
            this.uiControls = new UIControls(this);
            console.log('UIControls initialized successfully');
        } else {
            console.warn('UIControls class not found, creating dummy implementation');
            this.uiControls = {
                updateObjectProperties: function(obj) { 
                    console.log('Dummy updateObjectProperties called with:', obj);
                },
                clearObjectProperties: function() { 
                    console.log('Dummy clearObjectProperties called');
                },
                openSaveModal: function() { 
                    console.log('Dummy openSaveModal called');
                },
                openLoadModal: function() { 
                    console.log('Dummy openLoadModal called');
                }
            };
        }
    } catch (error) {
        ...
        this.uiControls = {
            updateObjectProperties: function(obj) { 
                console.log('Fallback updateObjectProperties called with:', obj);
            },
            clearObjectProperties: function() { 
                console.log('Fallback clearObjectProperties called');
            },
            openSaveModal: function() { 
                console.log('Fallback openSaveModal called');
            },
            openLoadModal: function() { 
                console.log('Fallback openLoadModal called');
            }
        };
    }
    ...
    // Populate furniture models by category
    const furnitureModels = vanBuilderData.availableModels.furniture;
    const categories = ['kitchen', 'sleeping', 'storage', 'bathroom'];
    
    categories.forEach(category => {
        const categoryContainer = $(`.category-items[data-category="${category}"]`);
        categoryContainer.empty();
        
        // Filter and add items for this category
        const categoryModels = furnitureModels.filter(model => model.category === category);
        ...
        categoryModels.forEach(model => {
            ...
            categoryContainer.append(modelElement);
        });
    });
...
$('.category-items').on('click', '.van-builder-model-item', function() {
    console.log('Category item clicked:', $(this).data('model-id'));
    const modelId = $(this).data('model-id');
...
this.sceneManager.onObjectSelected = (object) => {
    ...
    if (this.uiControls) {
        ...
        console.warn('UIControls not initialized');
    }
};
...
this.sceneManager.onObjectDeselected = () => {
   ...
   if (!modelId) {
    ...
    const furnitureModels = vanBuilderData.availableModels.furniture;
   if (!furnitureModels) {
       console.error('Furniture models not available');
       return;
   }
   ...
   if (!modelData) {
    ...
    if (modelData) {
        console.log('Loading model from:', modelData.file);
        this.modelLoader.loadModel(modelData.file, (model) => {
   if (!model) {
       console.error('Failed to load model:', modelData.file);
       return;
   }
            ...
            // Calculate bounding box to place object on the ground
            const box = new THREE.Box3().setFromObject(model);
            const height = box.max.y - box.min.y;
            // Adjust the position to place the bottom of the object on the grid
            model.position.y = Math.abs(box.min.y);
            
            // Position in front of camera but not too far
            const cameraDirection = new THREE.Vector3();
            this.sceneManager.camera.getWorldDirection(cameraDirection);
            cameraDirection.multiplyScalar(3); // 3 units in front of camera
            
            model.position.x = this.sceneManager.camera.position.x + cameraDirection.x;
            model.position.z = this.sceneManager.camera.position.z + cameraDirection.z;
